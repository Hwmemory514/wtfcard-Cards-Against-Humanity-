...
- 优化白牌分配：普通白牌在全房间内保持唯一，不会同时出现在多名玩家手中。
- 新增手牌锁定：锁定且未打出的牌会跨回合保留，其余位置自动补充新牌。
- 完善锁牌规则：每人最多锁定 4 张，选牌超时会自动打出第一张未锁定牌。
- 修复更新后旧服务仍在运行的问题：后端文件发生变化时，启动脚本会自动重启 Node 服务。

## 2026/8/9 更新

- 新增限时裁决：裁判需在 25 秒内选择赢家，超时后由系统随机选出一个有效答案。
- 新增限时选牌：玩家需在 30 秒内提交答案，超时后由系统自动打出其第一张手牌。
- 优化倒计时显示：通过网络校时保持截止时间一致，所有成员均可看到带进度条的阶段倒计时。
- 修复聊天区滚动：大量消息只在聊天面板内部滚动，不再持续撑高整个页面。
- 新增常用表情面板：内置 48 个高频 Emoji，可插入聊天输入框的光标位置。

## 2026/8/8 更新

- 更新游戏词库：现有 72 张黑卡和 96 张白卡。
- 新增首次启动脚本：自动准备 Node.js、cloudflared 和游戏依赖。
- 修复移动端语音播报：首次交互会静默启用，裁判点选答案时直接触发朗读。
